...
class Inv_stores_transferController extends Controller
{
    ...
    public function store(Inv_stores_transferRequest $request)
    {
        try {
            ...
            $CheckOpenOrder = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("auto_serial"), array("com_code" => $com_code, 'transfer_from_store_id' => $request->transfer_from_store_id, "transfer_to_store_id" => $request->transfer_to_store_id, 'is_approved' => 0));
            if (!empty($CheckOpenOrder)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا هناك بالفعل أمر تحويل مازال مفتوح بين المخزنين   !!'])
                    ->withInput();
            }
            $row = HelperClass::get_cols_where_row_orderby(new inv_stores_transfer(), array("auto_serial"), array("com_code" => $com_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            HelperClass::insert(new inv_stores_transfer(), $data_insert);
    ...
    public function show($id)
    {
        try {
            ...
            $data['notgiveStatecounter_details'] = HelperClass::get_count_where(new inv_stores_transfer_details(), array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $data['auto_serial'], 'is_approved' => 0, 'is_canceld_receive' => 0));
    ...
    public function edit($id)
    {
        ...
        if ($data['is_approved'] == 1) {
            return redirect()->route('inv_stores_transfer.index')->with(['error' => 'عفوا لايمكن التحديث علي فاتورة معتمدة ومؤرشفة']);
        }
        $stores = HelperClass::get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code), 'id', 'DESC');
        $added_counter_details = HelperClass::get_count_where(new inv_stores_transfer_details(), array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $data['auto_serial']));
    ...
    public function update($id, Inv_stores_transferRequestUpdate $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "auto_serial"), array("id" => $id, "com_code" => $com_code));
            if (empty($data)) {
                return redirect()->route('inv_stores_transfer.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
            }
            if ($data['is_approved'] == 1) {
            ...
            HelperClass::update(new inv_stores_transfer(), $data_to_update, array("id" => $id, "com_code" => $com_code));
            return redirect()->route('inv_stores_transfer.index')->with(['success' => 'لقد تم تحديث البيانات بنجاح']);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()])
                ->withInput();
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "auto_serial"), array("id" => $id, "com_code" => $com_code));
            ...
            $added_counter_details = HelperClass::get_count_where(new inv_stores_transfer_details(), array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $parent_pill_data['auto_serial']));
            if ($added_counter_details > 0) {
                return redirect()->back()
                    ->with(['error' => 'عفوا  لايمكن الحذف لوجوده اصناف بالفعل مضافة علي أمر التحويل!   ']);
            }
            //حنحذف الفاتورة الاب
            $flag = HelperClass::delete(new inv_stores_transfer(), array("id" => $id, "com_code" => $com_code));
            if ($flag) {
                HelperClass::delete(new inv_stores_transfer_details(), array("inv_stores_transfer_auto_serial" => $parent_pill_data['auto_serail'], "com_code" => $com_code));
            }
            return redirect()->route('inv_stores_transfer.index')->with(['success' => 'لقد تم حذف  البيانات بنجاح']);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()]);
        }
    ...
    public function load_modal_add_details(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "transfer_from_store_id"), array("auto_serial" => $request->autoserailparent, "com_code" => $com_code));
            if (!empty($parent_pill_data)) {
                if ($parent_pill_data['is_approved'] == 0) {
                    ...
                    $stores = HelperClass::get_cols_where(new Store(), array('id', 'name'), array('com_code' => $com_code, 'id' => $parent_pill_data['transfer_from_store_id']), 'id', 'DESC');
    ...
    public function Add_item_to_invoice(Request $request)
    {
        try {
            if ($request->ajax()) {
                $com_code = auth()->user()->com_code;
                $invoice_data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "order_date", "id", "transfer_from_store_id", "transfer_to_store_id"), array("com_code" => $com_code, "auto_serial" => $request->autoserailparent));
                        ...
                        if (!empty($batch_data)) {
                            if ($batch_data['quantity'] >= $request->item_quantity) {
                                ...
                                if (!empty($itemCard_Data)) {
                                    ...
                                    $flag_datainsert_items = HelperClass::insert(new inv_stores_transfer_details(), $datainsert_items, true);
    ...
    {
        $com_code = auth()->user()->com_code;
        $invoice_data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("auto_serial"), array("com_code" => $com_code, "auto_serial" => $auto_serial));
        if (!empty($invoice_data)) {
            //first get sum of details
            $dataUpdateParent['total_cost_items'] = HelperClass::get_sum_where(new inv_stores_transfer_details(), "total_price", array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $auto_serial));
            $dataUpdateParent['items_counter'] = HelperClass::get_sum_where(new inv_stores_transfer_details(), "deliverd_quantity", array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $auto_serial));
            $dataUpdateParent['updated_at'] = date("Y-m-d H:i:s");
            $dataUpdateParent['updated_by'] = auth()->user()->com_code;
            HelperClass::update(new inv_stores_transfer(), $dataUpdateParent, array("com_code" => $com_code, "auto_serial" => $auto_serial));
        }
    }
    public function reload_parent_pill(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("*"), array("auto_serial" => $request->autoserailparent, "com_code" => $com_code));
            if (!empty($data)) {
                $data['notgiveStatecounter_details'] = HelperClass::get_count_where(new inv_stores_transfer_details(), array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $data['auto_serial'], 'is_approved' => 0, 'is_canceld_receive' => 0));
                $data['added_by_admin'] = Admin::where('id', $data['added_by'])->value('name');
                $data['from_store_name'] = Store::where('id', $data['transfer_from_store_id'])->value('name');
    ...
    public function reload_itemsdetials(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "id"), array("auto_serial" => $auto_serial, "com_code" => $com_code));
            $details = HelperClass::get_cols_where(new inv_stores_transfer_details(), array("*"), array('inv_stores_transfer_auto_serial' => $auto_serial, 'com_code' => $com_code), 'id', 'DESC');
            if (!empty($details)) {
                foreach ($details as $info) {
                    $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                    $info->uom_name = HelperClass::get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    public function delete_details($id, $parent_id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $parent_pill_data = HelperClass::get_cols_where_row(new inv_stores_transfer(), array("is_approved", "auto_serial", "transfer_from_store_id", "transfer_to_store_id"), array("id" => $parent_id, "com_code" => $com_code));
            if (empty($parent_pill_data)) {
                return redirect()->back()
                    ->with(['error' => ' عفوا حدث خطأ ما']);
            }
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    ...
                    $batch_data = HelperClass::get_cols_where_row(new Inv_itemcard_batches(), array("quantity", "unit_cost_price", "id", "production_date", "expired_date"), array("com_code" => $com_code, "auto_serial" => $item_row['transfer_from_batch_id'], 'store_id' => $parent_pill_data['transfer_from_store_id'], 'item_code' => $item_row['item_code']));
    ...
    public function do_approve($id)
    {
        try {
            ...
            $notgiveStatecounter_details = HelperClass::get_count_where(new inv_stores_transfer_details(), array("com_code" => $com_code, "inv_stores_transfer_auto_serial" => $parent_pill_data['auto_serial'], 'is_approved' => 0, 'is_canceld_receive' => 0));
            ...
            $flag = HelperClass::update(new inv_stores_transfer(), $DataToUpdate, array("id" => $id, "com_code" => $com_code));
            return redirect()->route('inv_stores_transfer.index')->with(['success' => 'لقد تم اغلاق وأرشفة الامر بنجاح  البيانات بنجاح']);
        } catch (\Exception $ex) {
            return redirect()->back()
                ->with(['error' => 'عفوا حدث خطأ ما' . $ex->getMessage()]);
        }
