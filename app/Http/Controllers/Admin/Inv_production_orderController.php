...
class Inv_production_orderController extends Controller
{
    public function index()
    {
        try {
            ...
            $data = HelperClass::get_cols_where_p(new Inv_production_order(), array("*"), array("com_code" => $com_code), 'id', 'DESC', PAGINATION_COUNT);
            if (!empty($data)) {
    ...
    public function store(inv_production_orderRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $row = HelperClass::get_cols_where_row_orderby(new Inv_production_order(), array("auto_serial"), array("com_code" => $com_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            HelperClass::insert(new Inv_production_order(), $data);
    ...
    {
        $com_code = auth()->user()->com_code;
        $data = HelperClass::get_cols_where_row(new Inv_production_order(), array("*"), array("id" => $id, "com_code" => $com_code));
        if (empty($data)) {
    ...
    public function update($id, inv_production_orderRequest $request)
    {
        try {
            ...
            HelperClass::update(new Inv_production_order(), $data_to_update, array("id" => $id, "com_code" => $com_code));
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new Inv_production_order(), array("is_closed"), array("id" => $id, "com_code" => $com_code));
            if (empty($data)) {
                return redirect()->back()
            ...
            $flag = HelperClass::delete(new Inv_production_order(), array("id" => $id, "com_code" => $com_code));
            if ($flag) {
    ...
    public function show_more_detials(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new  Inv_production_order(), array("*"), array("id" => $request->id, "com_code" => $com_code));
            if (!empty($data)) {
    ...
    public function do_approve($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new Inv_production_order(), array("is_approved"), array("id" => $id, "com_code" => $com_code));
            if (empty($data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
            ...
            $flag = HelperClass::update(new Inv_production_order(), $data_to_update, array("id" => $id, "com_code" => $com_code));
    ...
    public function do_closes_archive($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = HelperClass::get_cols_where_row(new Inv_production_order(), array("is_approved", "is_closed"), array("id" => $id, "com_code" => $com_code));
            if (empty($data)) {
                return redirect()->back()
                    ->with(['error' => 'عفوا حدث خطأ ما']);
            }
