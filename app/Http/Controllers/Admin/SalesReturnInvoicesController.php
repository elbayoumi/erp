...
class SalesReturnInvoicesController extends Controller
{
    public function index()
    {
        ...
        $data = HelperClass::get_cols_where_p(new SalesReturn(), array("*"), array("com_code" => $com_code), "id", "DESC", PAGINATION_COUNT);
        if (!empty($data)) {
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            $last_auto_serial_Date = HelperClass::get_cols_where_row_orderby(new SalesReturn(), array("auto_serial"), array("com_code" => $com_code), 'id', 'DESC');
            ...
            $flag = HelperClass::insert(new SalesReturn(), $data_insert, false);
            if ($flag) {
    ...
    public function load_invoice_update_modal(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $invoice_data = HelperClass::get_cols_where_row(new SalesReturn(), array("*"), array("com_code" => $com_code, "auto_serial" => $request->auto_serial));
            $stores = HelperClass::get_cols_where(new Store(), array("id", "name"), array("com_code" => $com_code, "active" => 1), 'id', 'ASC');
            $user_shift = HelperClass::get_user_shift(new Admins_Shifts(), new Treasuries(), new Treasuries_transactions());
            $delegates = HelperClass::get_cols_where(new Delegate(), array("delegate_code", "name"), array("com_code" => $com_code, "active" => 1));
            ...
            $Sales_matrial_types = HelperClass::get_cols_where(new Sales_matrial_types(), array("id", "name"), array("com_code" => $com_code, "active" => 1));
            $sales_invoices_details = HelperClass::get_cols_where(new SalesReturnDetails(), array("*"), array("com_code" => $com_code, "sales_invoices_auto_serial" => $request->auto_serial));
    ...
    public function Add_item_to_invoice(Request $request)
    {
        try {
            if ($request->ajax()) {
                $com_code = auth()->user()->com_code;
                $invoice_data = HelperClass::get_cols_where_row(new SalesReturn(), array("is_approved", "invoice_date", "is_has_customer", "customer_code", "id"), array("com_code" => $com_code, "auto_serial" => $request->invoiceautoserial));
                if (!empty($invoice_data)) {
                    if ($invoice_data['is_approved'] == 0) {
                        ...
                        if (!empty($itemCard_Data)) {
                            ...
                            $flag_datainsert_items = HelperClass::insert(new SalesReturnDetails(), $datainsert_items, true);
                            if (!empty($flag_datainsert_items)) {
                                ...
                                if ($datainsert_items['batch_auto_serial'] == "") {
                                    if ($datainsert_items['sales_item_type'] == 2) {
                                        $batch_data = HelperClass::get_cols_where_row(
                                            new Inv_itemcard_batches(),
                                    ...
                                    if (empty($batch_data)) {
                                        ...
                                        $flag = HelperClass::insert(new Inv_itemcard_batches(), $dataInsertBatch);
                                        if ($flag) {
                                            $dataToupdateParentDetails['batch_auto_serial'] = $dataInsertBatch['auto_serial'];
                                            HelperClass::update(new SalesReturnDetails(), $dataToupdateParentDetails, $datainsert_items);
                                        }
                                    }
                                    //end
                                } else {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    ...
                    HelperClass::update(new SalesReturn(), $dataUpdateParent, array("com_code" => $com_code, "auto_serial" => $request->auto_serial));
                    echo json_encode("done");
                }
            }
        }
    ...
    function remove_active_row_item(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $invoice_data = HelperClass::get_cols_where_row(new SalesReturn(), array("is_approved", "is_has_customer", "customer_code"), array("com_code" => $com_code, "auto_serial" => $request->auto_serial));
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    $sales_invoices_details_data = HelperClass::get_cols_where_row(new SalesReturnDetails(), array("batch_auto_serial", "quantity", "item_code", "store_id", "isparentuom"), array("com_code" => $com_code, "id" => $request->id));
                    if (!empty($sales_invoices_details_data)) {
                        ...
                        if (!empty($batch_data)) {
                            ...
                            if (!empty($itemCard_Data)) {
                                ...
                                $flag = HelperClass::delete(new SalesReturnDetails(), array("com_code" => $com_code, "id" => $request->id));
                                if ($flag) {
                                    //رد الكمية الي الباتش
                                    //كمية الصنف بكل المخازن قبل الحركة
                                    $quantityBeforMove = HelperClass::get_sum_where(
    ...
    function DoApproveInvoiceFinally(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $invoice_data = HelperClass::get_cols_where_row(new SalesReturn(), array("is_approved", "pill_type", "total_cost", "customer_code", "is_has_customer"), array("com_code" => $com_code, "auto_serial" => $request->auto_serial));
            if (!empty($invoice_data)) {
                if ($invoice_data['is_approved'] == 0) {
                    ...
                    if ($invoice_data['is_has_customer'] == 1) {
                        $customerData = HelperClass::get_cols_where_row(new Customer(), "account_number", array("com_code" => $com_code, "customer_code" => $invoice_data['customer_code']));
                        $dataUpdateParent['account_number'] = $customerData['account_number'];
                    }
                    $flag = HelperClass::update(new SalesReturn(), $dataUpdateParent, array("com_code" => $com_code, "auto_serial" => $request->auto_serial));
                    if ($flag) {
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $SalesInvoiceData = HelperClass::get_cols_where_row(new SalesReturn(), array("is_approved", "auto_serial"), array("id" => $id, "com_code" => $com_code));
            if (!empty($SalesInvoiceData)) {
                $flag = HelperClass::delete(new SalesReturn(), array("id" => $id, "com_code" => $com_code));
                if ($flag) {
                    HelperClass::delete(new SalesReturnDetails(), array("sales_invoices_auto_serial" => $SalesInvoiceData['auto_serial'], "com_code" => $com_code));
    ...
    {
        $com_code = auth()->user()->com_code;
        $invoice_data = HelperClass::get_cols_where_row(new SalesReturn(), array("*"), array("com_code" => $com_code, "id" => $id));
        if (empty($invoice_data)) {
        ...
        $sales_invoices_details = HelperClass::get_cols_where(new SalesReturnDetails(), array("*"), array("com_code" => $com_code, "sales_invoices_auto_serial" => $invoice_data['auto_serial']));
        if (!empty($sales_invoices_details)) {
            foreach ($sales_invoices_details  as $info) {
                $info->store_name = HelperClass::get_field_value(new Store(), "name", array("com_code" => $com_code, "id" => $info->store_id));
                $info->item_name = HelperClass::get_field_value(new Inv_itemCard(), "name", array("com_code" => $com_code, "item_code" => $info->item_code));
