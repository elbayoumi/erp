...
class InvItemCardController extends Controller
{
    public function index()
    {
        ...
        $data = HelperClass::get_cols_where_p(new Inv_itemCard(), array("*"), array("com_code" => $com_code), 'id', 'DESC', PAGINATION_COUNT);
        if (!empty($data)) {
            foreach ($data as $info) {
                $info->added_by_admin = HelperClass::get_field_value(new Admin(), 'name', array('id' => $info->added_by));
                ...
                if ($info->updated_by > 0 and $info->updated_by != null) {
                    $info->updated_by_admin = HelperClass::get_field_value(new Admin(), 'name', array('id' => $info->updated_by));
                }
            }
        }
    ...
    public function create()
    {
        ...
        $inv_itemcard_categories = HelperClass::get_cols_where(new inv_itemcard_categorie(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
        $inv_uoms_parent = HelperClass::get_cols_where(new Inv_uom(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1, 'is_master' => 1), 'id', 'DESC');
        $inv_uoms_child = HelperClass::get_cols_where(new Inv_uom(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1, 'is_master' => 0), 'id', 'DESC');
        $item_card_data = HelperClass::get_cols_where(new Inv_itemCard(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
    ...
    public function store(ItemcardRequest $request)
    {
        try {
            ...
            $row = HelperClass::get_cols_where_row_orderby(new Inv_itemCard(), array("item_code"), array("com_code" => $com_code), 'id', 'DESC');
            if (!empty($row)) {
            ...
            if ($request->has('Item_img')) {
                ...
                $the_file_path = HelperClass::uploadImage('assets/admin/uploads', $request->Item_img);
    ...
    {
        $data = HelperClass::get_cols_where_row(new Inv_itemCard(), array("*"), array("id" => $id));
        $com_code = auth()->user()->com_code;
        $inv_itemcard_categories = HelperClass::get_cols_where(new inv_itemcard_categorie(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
        $inv_uoms_parent = HelperClass::get_cols_where(new Inv_uom(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1, 'is_master' => 1), 'id', 'DESC');
        $inv_uoms_child = HelperClass::get_cols_where(new Inv_uom(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1, 'is_master' => 0), 'id', 'DESC');
        $item_card_data = HelperClass::get_cols_where(new Inv_itemCard(), array('id', 'name'), array('com_code' => $com_code, 'active' => 1), 'id', 'DESC');
        $counterUsedin_with_suppliers = HelperClass::get_count_where(new Suppliers_with_orders_details(), array("com_code" => $com_code, "item_code" => $data['item_code']));
        $counterUsedin_with_sales = HelperClass::get_count_where(new Sales_invoices_details(), array("com_code" => $com_code, "item_code" => $data['item_code']));
    ...
    public function update($id, ItemcardRequestUpdate $request)
    {
        try {
            ...
            HelperClass::update(new Inv_itemCard(), $data_to_update, array('id' => $id, 'com_code' => $com_code));
    ...
    public function delete($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $item_row = HelperClass::get_cols_where_row(new Inv_itemCard(), array("id"), array("id" => $id, 'com_code' => $com_code));
            if (!empty($item_row)) {
                $flag = HelperClass::delete(new Inv_itemCard(), array("id" => $id, 'com_code' => $com_code));
                if ($flag) {
                    return redirect()->back()
    ...
    public function show($id)
    {
        $data = HelperClass::get_cols_where_row(new Inv_itemCard(), array("*"), array("id" => $id));
        $com_code = auth()->user()->com_code;
        $data['added_by_admin'] = HelperClass::get_field_value(new Admin(), 'name', array('id' => $data['added_by']));
        $data['inv_itemcard_categories_name'] = HelperClass::get_field_value(new inv_itemcard_categorie(), 'name', array('id' => $data['inv_itemcard_categories_id']));
        $data['parent_item_name'] = HelperClass::get_field_value(new Inv_itemCard(), 'name', array('id' => $data['parent_inv_itemcard_id']));
        $data['Uom_name'] = HelperClass::get_field_value(new Inv_uom(), 'name', array('id' => $data['uom_id']));
        if ($data['does_has_retailunit'] == 1) {
            $data['retail_uom_name'] = HelperClass::get_field_value(new Inv_uom(), 'name', array('id' => $data['retail_uom_id']));
        }
        if ($data['updated_by'] > 0 and $data['updated_by']  != null) {
            $data['updated_by_admin'] = HelperClass::get_field_value(new Admin(), 'name', array('id' => $data['updated_by']));
        }
        $inv_itemcard_movements_categories = HelperClass::get_cols(new Inv_itemcard_movements_categories(), array("*"), "id", "ASC");
        $inv_itemcard_movements_types = HelperClass::get_cols(new Inv_itemcard_movements_types(), array("*"), "id", "ASC");
        $stores = HelperClass::get_cols_where(new Store(), array("id", "name"), array("com_code" => $com_code), "id", "ASC");
        ...
        if ($request->ajax()) {
            ...
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = HelperClass::get_field_value(new Admin(), 'name', array('id' => $info->added_by));
                    $info->inv_itemcard_categories_name = HelperClass::get_field_value(new inv_itemcard_categorie(), 'name', array('id' => $info->inv_itemcard_categories_id));
                    $info->parent_item_name = HelperClass::get_field_value(new Inv_itemCard(), 'name', array('id' => $info->parent_inv_itemcard_id));
                    $info->Uom_name = HelperClass::get_field_value(new Inv_uom(), 'name', array('id' => $info->uom_id));
                    $info->retail_uom_name = HelperClass::get_field_value(new Inv_uom(), 'name', array('id' => $info->retail_uom_id));
                    if ($info->updated_by > 0 and $info->updated_by != null) {
                        $info->updated_by_admin = HelperClass::get_field_value(new Admin(), 'name', array('id' => $info->updated_by));
    ...
    public function ajax_search_movements(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->added_by_admin = HelperClass::get_field_value(new Admin(), 'name', array('id' => $info->added_by));
                    $info->inv_itemcard_movements_categories_name = HelperClass::get_field_value(new Inv_itemcard_movements_categories(), 'name', array('id' => $info->inv_itemcard_movements_categories));
                    $info->inv_itemcard_movements_types_name = HelperClass::get_field_value(new Inv_itemcard_movements_types(), 'type', array('id' => $info->items_movements_types));
                    $info->store_name = HelperClass::get_field_value(new Store(), 'name', array('id' => $info->store_id));
                }
            }
    ...
    {
        $data = HelperClass::get_cols_where_row(new Inv_itemCard(), array("barcode", "name"), array("id" => $id));
        if (empty($data)) {
            return redirect()->route('itemcard.index')->with(['error' => 'عفوا غير قادر علي الوصول الي البيانات المطلوبة !!']);
        }
