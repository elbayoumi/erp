...
class FinancialReportController extends Controller
{
    ...
    public function supplier_account_mirror(Request $request)
    {
        if ($_POST) {
            ...
            $supplierData = HelperClass::get_cols_where_row(new Supplier(), array("account_number", "start_balance", "name", "phones", "date", "suuplier_code"), array("com_code" => $com_code, 'suuplier_code' => $request->suuplier_code));
            if (empty($supplierData)) {
                return redirect()->back()->with(['error' => 'عفوا غير قادر علي الوصول الي بيانات هذا المورد !!']);
            }
            //General Report
            if ($request->report_type == 1) {
                $supplierData['the_final_Balance'] = HelperClass::refresh_account_blance_supplier($supplierData['account_number'], new Account(), new Supplier(), new Treasuries_transactions(), new Suppliers_with_orders(), new services_with_orders(), true);
                ...
                if ($supplierData['Does_show_items'] == 1) {
                    foreach ($details['Burchases'] as $Bur) {
                        $Bur->itemsdetails = HelperClass::get_cols_where(new Suppliers_with_orders_details(), array("*"), array("com_code" => $com_code, "suppliers_with_orders_auto_serial" => $Bur->auto_serial, 'order_type' => $Bur->order_type));
                ...
                if ($supplierData['Does_show_items'] == 1) {
                    foreach ($details['BurchasesReturn'] as $return) {
                        $return->itemsdetails = HelperClass::get_cols_where(new Suppliers_with_orders_details(), array("*"), array("com_code" => $com_code, "suppliers_with_orders_auto_serial" => $return->auto_serial, 'order_type' => $return->order_type));
                ...
                if (!empty($details['Treasuries_transactions'])) {
                    foreach ($details['Treasuries_transactions'] as $info) {
                        ...
                        $info->mov_type_name = HelperClass::get_field_value(new Mov_type(), 'name', array('id' => $info->mov_type));
                    }
                }
                ...
                if ($supplierData['Does_show_items'] == 1) {
                    if (!empty($details['services_orders'])) {
                        foreach ($details['services_orders'] as $info) {
                            $info->ServicesDetails = HelperClass::get_cols_where(new services_with_orders_details(), array("*"), array("com_code" => $com_code, 'services_with_orders_auto_serial' => $info->auto_serial, 'order_type' => $info->order_type));
            ...
            $com_code = auth()->user()->com_code;
            $suppliers = HelperClass::get_cols_where(new Supplier(), array("suuplier_code", "name", "date"), array("com_code" => $com_code));
    ...
    public function customer_account_mirror(Request $request)
    {
        if ($_POST) {
            $com_code = auth()->user()->com_code;
            $CustomerData = HelperClass::get_cols_where_row(new Customer(), array("account_number", "start_balance", "name", "phones", "date", "customer_code"), array("com_code" => $com_code, 'customer_code' => $request->customer_code));
            if (empty($CustomerData)) {
                return redirect()->back()->with(['error' => 'عفوا غير قادر علي الوصول الي بيانات هذا العميل !!']);
            }
            //General Report لو تقرير اجمالي
            if ($request->report_type == 1) {
                $CustomerData['the_final_Balance'] = HelperClass::refresh_account_blance_customer($CustomerData['account_number'], new Account(), new Customer(), new Treasuries_transactions(), new Sales_invoices(), new SalesReturn(), new services_with_orders(), true);
                ...
                if ($CustomerData['Does_show_items'] == 1) {
                    foreach ($details['sales'] as $Bur) {
                        $Bur->itemsdetails = HelperClass::get_cols_where(new Sales_invoices_details(), array("*"), array("com_code" => $com_code, "sales_invoices_auto_serial" => $Bur->auto_serial));
                        if (!empty($Bur->itemsdetails)) {
                            foreach ($Bur->itemsdetails as $info) {
                                $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                                $info->uom_name = HelperClass::get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
                ...
                if ($CustomerData['Does_show_items'] == 1) {
                    foreach ($details['sales_return'] as $return) {
                        $return->itemsdetails = HelperClass::get_cols_where(new SalesReturnDetails(), array("*"), array("com_code" => $com_code, "sales_invoices_auto_serial" => $return->auto_serial));
                        if (!empty($return->itemsdetails)) {
                            foreach ($return->itemsdetails as $info) {
                                $info->item_card_name = Inv_itemCard::where('item_code', $info->item_code)->value('name');
                                $info->uom_name = HelperClass::get_field_value(new Inv_uom(), "name", array("id" => $info->uom_id));
    ...
    public function delegate_account_mirror(Request $request)
    {
        if ($_POST) {
            $com_code = auth()->user()->com_code;
            $delegateData = HelperClass::get_cols_where_row(new Delegate(), array("account_number", "start_balance", "name", "phones", "date", "delegate_code"), array("com_code" => $com_code, 'delegate_code' => $request->delegate_code));
            if (empty($delegateData)) {
                return redirect()->back()->with(['error' => 'عفوا غير قادر علي الوصول الي بيانات هذا العميل !!']);
            }
            //General Report لو تقرير اجمالي
            if ($request->report_type == 1) {
                $delegateData['the_final_Balance'] = HelperClass::refresh_account_blance_delegate($delegateData['account_number'], new Account(), new Delegate(), new Treasuries_transactions(), new Sales_invoices(), new services_with_orders(), true);
            ...
            $com_code = auth()->user()->com_code;
            $delegates = HelperClass::get_cols_where(new Delegate(), array('delegate_code', 'name', 'date'), array('com_code' => $com_code));
