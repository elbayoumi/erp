            ...
            <div class="row">
                        ...
                        <select id="item_code_add" class="form-control select2" style="width: 100%;">
                            ...
                            @if (@isset($item_cards) && !@empty($item_cards))
                                @foreach ($item_cards as $info)
                                    <option @if ($item_data_detials['item_code'] == $info->item_code) selected="selected" @endif
                                        data-type="{{ $info->item_type }}" value="{{ $info->item_code }}">
                                        {{ $info->name }} - {{ $info->barcode }} </option>
                                @endforeach
                            @endif
                        </select>
                        @error('suuplier_code')
                        ...
                        <select id="uom_id_Add" class="form-control select2" style="width: 100%;">
                            ...
                            @if (@isset($item_card_Data) && !@empty($item_card_Data))
                                @if ($item_card_Data['does_has_retailunit'] == 1)
                                    <option @if ($item_card_Data['uom_id'] == $item_data_detials['uom_id']) selected @endif data-isparentuom="1"
                                        value="{{ $item_card_Data['uom_id'] }}">
                                        {{ $item_card_Data['parent_uom_name'] }} (وحده اب) </option>
                                    <option @if ($item_card_Data['retail_uom_id'] == $item_data_detials['uom_id']) selected @endif data-isparentuom="0"
                                        value="{{ $item_card_Data['retail_uom_id'] }}">
                ...
                <div class="col-md-4 relatied_to_itemCard">
                    <div class="form-group">
                        ...
                        <input oninput="this.value=this.value.replace(/[^0-9]/g,'');" id="quantity_edit"
                            class="form-control" value="{{ $item_data_detials['deliverd_quantity'] * 1 }}"
                ...
                <div class="col-md-4 relatied_to_itemCard">
                    <div class="form-group">
                        ...
                        <input oninput="this.value=this.value.replace(/[^0-9]/g,'');" id="price_edit"
                            class="form-control" value="{{ $item_data_detials['unit_price'] * 1 }}"
                            oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
                            onchange="try{setCustomValidity('')}catch(e){}">
                    </div>
                </div>
                <div class="col-md-4 relatied_to_date" @if ($item_data_detials['item_card_type'] != 2) style="display: none;" @endif>
                    <div class="form-group">
                        ...
                        <input type="date" id="production_date" class="form-control"
                            value="{{ $item_data_detials['production_date'] }}"
                            oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
                            onchange="try{setCustomValidity('')}catch(e){}">
                    </div>
                </div>
                <div class="col-md-4 relatied_to_date" @if ($item_data_detials['item_card_type'] != 2) style="display: none;" @endif>
                    <div class="form-group">
                        ...
                        <input type="date" id="expire_date" class="form-control"
                            value="{{ $item_data_detials['expire_date'] }}"
                            oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
                            onchange="try{setCustomValidity('')}catch(e){}">
                    </div>
                </div>
                <div class="col-md-4 relatied_to_itemCard">
                    <div class="form-group">
                        ...
                        <input readonly id="total_edit" class="form-control"
                            value="{{ $item_data_detials['total_price'] * 1 }}"
                            oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
                            onchange="try{setCustomValidity('')}catch(e){}">
                    </div>
                </div>
