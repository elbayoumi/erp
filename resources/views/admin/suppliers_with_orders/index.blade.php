...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <input type="hidden" id="ajax_search_url" value="{{ route('suppliers_orders.ajax_search') }}">
            <a href="{{ route('suppliers_orders.create') }}" class="btn btn-sm btn-success">اضافة جديد</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($info->is_approved == 0)
                                                    <a href="{{ route('suppliers_orders.edit', $info->id) }}"
                                                        class="btn btn-sm  btn-primary">تعديل</a>
                                                    <a href="{{ route('suppliers_orders.delete', $info->id) }}"
                                                        class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                                                @endif
                                                <a href="{{ route('suppliers_orders.show', $info->id) }}"
                                                    class="btn btn-sm   btn-info">التفاصيل</a>
                                                <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white"
                                                    target="_blank"
                                                    href="{{ route('suppliers_orders.printsaleswina4', [$info->id, 'A4']) }}"
                                                    class="btn btn-primary btn-xs"> WA4</a>
                                                <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white"
                                                    target="_blank"
                                                    href="{{ route('suppliers_orders.printsaleswina4', [$info->id, 'A6']) }}"
