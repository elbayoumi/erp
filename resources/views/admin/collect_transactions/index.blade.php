...
<div class="card">
   <div class="card-header">
      ...
      <input type="hidden" id="ajax_url_get_account_blance" value="{{route('collect_transaction.get_account_blance')}}">
      <input type="hidden" id="ajax_url_ajax_search" value="{{route('collect_transaction.ajax_search')}}">
   </div>
   <!-- /.card-header -->
   <div class="card-body">
      @if(!@empty($checkExistsOpenShift))
      <form action="{{ route('collect_transaction.store') }}" method="post"  class="custom_form">
         <div class="row">
            @csrf
            <div class="col-md-4 >
