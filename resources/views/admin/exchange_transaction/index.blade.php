...
<a href="{{ route('collect_transaction.index') }}"> شاشه صرف النقدية </a>
@endsection
...
<div class="card">
   <div class="card-header">
      ...
      <input type="hidden" id="ajax_url_get_account_blance" value="{{route('exchange_transaction.get_account_blance')}}">
      <input type="hidden" id="ajax_url_ajax_search" value="{{route('exchange_transaction.ajax_search')}}">
   </div>
   <!-- /.card-header -->
   <div class="card-body">
      @if(!@empty($checkExistsOpenShift))
      <form action="{{ route('exchange_transaction.store') }}" method="post"  class="custom_form">
         <div class="row">
            @csrf
            <div class="col-md-4 >
      ...
      <table id="example2" class="table table-bordered table-hover">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td>
                  <a href="{{ route('treasuries.edit',$info->id) }}" class="btn btn-sm  btn-primary">طباعة</a>
                  <a href="{{ route('treasuries.details',$info->id) }}" class="btn btn-sm  btn-info">المزيد</a>
               </td>
            </tr>
            @endforeach
         </tbody>
