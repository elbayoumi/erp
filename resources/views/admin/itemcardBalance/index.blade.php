...
@section('contentheaderlink')
    <a href="{{ route('itemcardBalance.index') }}"> الأرصدة </a>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <input type="hidden" id="ajax_url_ajax_search" value="{{ route('itemcardBalance.ajax_search') }}">
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4" id="BatchquantitySerachDiv" style="display: none;">
                    <div class="form-group">
                        ...
                        <input oninput="this.value=this.value.replace(/[^0-9.]/g,'');" name="BatchquantitySerach"
                            id="BatchquantitySerach" class="form-control" value="">
                        @error('active')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                            ...
                            <tbody>
                                @foreach ($allitemscardData as $info)
                                    <tr>
                                        ...
                                        <td>{{ $info->name }}</td>
                                        <td>
                                            كل الكمية بالمخازن ( {{ $info->All_QUENTITY * 1 }} {{ $info->Uom_name }} ) <br>
                                            <br>
                                            <h3 style="text-align: center; font-size:15px; color:brown;">تفاصيل كميات الصنف
                                                بالمخازن</h3>
                                                ...
                                                <table id="example2" class="table table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        @foreach ($info->allBathces as $Det)
                                                            <tr>
                                                                <td
                                                                    @if ($Det->quantity == 0) class="bg-danger" @endif>
                                                                    {{ $Det->auto_serial }}</td>
                                                                <td>{{ $Det->store_name }}</td>
                                                                <td>
                                                                    عدد ( {{ $Det->quantity * 1 }} ) {{ $info->Uom_name }}
                                                                    بإجمالي تكلفة ( {{ $Det->total_cost_price * 1 }} جنيه )
                                                                    <br>
                                                                    بسعر ( {{ $Det->unit_cost_price * 1 }} جنيه ) لوحدة
                                                                    {{ $info->Uom_name }}
                                                                    @if ($info->does_has_retailunit == 1)
                                                                        <br>
                                                                        @if ($info->item_type == 2)
                                                                            تاريخ انتاج ( {{ $Det->production_date }} )
                                                                            <br>
                                                                        ...
                                                                        <span style="color: brown;"> مايوزاي بوحدة
                                                                            التجزئة</span>
                                                                        <br>
                                                                        عدد ( {{ $Det->qunatityRetail * 1 }} )
                                                                        {{ $info->retail_uom_name }} بإجمالي تكلفة
                                                                        ({{ $Det->total_cost_price * 1 }} جنيه) <br>
                                                                        بسعر ( {{ $Det->priceRetail * 1 }} جنيه ) لوحدة
                                                                        {{ $info->retail_uom_name }}
                                                                    @endif
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                            ...
                                            @else
                                                <h3 style="text-align: center; font-size:13px; color:brown;"> لاتوجد باتشات
                                                    لهذا الصنف</h3>
                                            @endif
                                        </td>
                                    </tr>
