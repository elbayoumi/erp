...
<a href="{{ route('admin_shift.index') }}">  شفتات الخزن </a>
@endsection
...
<div class="card">
   <div class="card-header">
      ...
      <!--  <input type="hidden" id="ajax_search_url" value="{{ route('admin_shift.ajax_search') }}"> -->
     <input type="hidden" id="ajax_review_now_url" value="{{ route('admin_shift.review_now') }}">

      @if(empty($checkExistsOpenShift))
      <a href="{{ route('admin_shift.create') }}" class="btn btn-sm btn-success" > فتح شفت جديد</a>
      @endif
   </div>
   <!-- /.card-header -->
   <div class="card-body">
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td>
                           ...
                           @if($info->is_finished==0 and $info->admin_id==auth()->user()->id)
                           <a href="{{ route('admin_shift.finish',$info->id) }}" class="btn btn-sm  are_you_shue btn-success">  انهاء الشفت</a>
                           @endif
                           <a target="_blank" href="{{ route('admin_shift.print_details',$info->id) }}" class="btn btn-sm  btn-primary"> طباعة الكشف</a>
