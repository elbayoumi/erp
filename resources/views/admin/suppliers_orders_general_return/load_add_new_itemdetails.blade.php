...
<div class="row">
            ...
            <select name="store_id" id="store_id" class="form-control ">
                @if (@isset($stores) && !@empty($stores))
                    ...
                    @foreach ($stores as $info)
                        <option @if ($i == 1) selected @endif value="{{ $info->id }}">
                            {{ $info->name }} </option>
                        @php $i++;  @endphp
                    @endforeach
                @else
            ...
            <select id="item_code" name="item_code" class="form-control select2" style="width: 100%;">
                ...
                @if (@isset($item_cards) && !@empty($item_cards))
                    @foreach ($item_cards as $info)
                        <option data-type="{{ $info->item_type }}" value="{{ $info->item_code }}"> {{ $info->name }} -
                            {{ $info->barcode }} </option>
                    @endforeach
                @endif
            </select>
    ...
    <div class="col-md-4 relatied_to_itemCard" style="display: none;">
        <div class="form-group">
            ...
            <input oninput="this.value=this.value.replace(/[^0-9]/g,'');" id="item_price" class="form-control"
                value="" oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
    ...
    <div class="col-md-4 relatied_to_itemCard" style="display: none;">
        <div class="form-group">
            ...
            <input oninput="this.value=this.value.replace(/[^0-9]/g,'');" id="item_quantity" class="form-control"
                value="" oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')"
                onchange="try{setCustomValidity('')}catch(e){}">
        </div>
    </div>
    <div class="col-md-4 relatied_to_itemCard" style="display: none;">
        <div class="form-group">
            ...
            <input readonly id="item_total" class="form-control" value=""
                oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')" onchange="try{setCustomValidity('')}catch(e){}">
        </div>
    </div>
    <div class="col-md-2">
        <div class="form-group text-center">
            <button type="button" style="margin-top: 31px;" class="btn btn-sm btn-danger"
                id="AddItemToIvoiceDetailsActive">اضف للفاتورة</button>
        </div>
    </div>
