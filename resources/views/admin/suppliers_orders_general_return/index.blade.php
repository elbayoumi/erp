...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <input type="hidden" id="ajax_search_url" value="{{ route('suppliers_orders_general_return.ajax_search') }}">
            <a href="{{ route('suppliers_orders_general_return.create') }}" class="btn btn-sm btn-success">اضافة جديد</a>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input name="order_date_form" id="order_date_form" class="form-control" type="date"
                            value="">
                    </div>
                </div>
                <div class="col-md-4">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if ($info->is_approved == 0)
                                                    <a href="{{ route('suppliers_orders_general_return.edit', $info->id) }}"
                                                        class="btn btn-sm  btn-primary">تعديل</a>
                                                    <a href="{{ route('suppliers_orders_general_return.delete', $info->id) }}"
                                                        class="btn btn-sm are_you_shue  btn-danger">حذف</a>
                                                @endif
                                                <a href="{{ route('suppliers_orders_general_return.show', $info->id) }}"
                                                    class="btn btn-sm   btn-info">التفاصيل</a>
                                                <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white"
                                                    target="_blank"
                                                    href="{{ route('suppliers_orders_general_return.printsaleswina4', [$info->id, 'A4']) }}"
                                                    class="btn btn-primary btn-xs"> WA4</a>
                                                <a style="font-size: .875rem; padding: 0.25rem 0.5rem;color:white"
                                                    target="_blank"
                                                    href="{{ route('suppliers_orders_general_return.printsaleswina4', [$info->id, 'A6']) }}"
