                ...
                <div class="card-body">
                    @if (!@empty($data))
                        @if ($data['is_approved'] == 0)
                            <form action="{{ route('suppliers_orders_general_return.update', $data['id']) }}" method="post">
                                @csrf
                                    ...
                                    <select name="suuplier_code" id="suuplier_code" class="form-control select2">
                                        ...
                                        @if (@isset($suupliers) && !@empty($suupliers))
                                            @foreach ($suupliers as $info)
                                                <option @if (old('suuplier_code', $data['suuplier_code']) == $info->suuplier_code) selected="selected" @endif
