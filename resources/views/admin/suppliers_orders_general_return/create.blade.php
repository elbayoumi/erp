    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form action="{{ route('suppliers_orders_general_return.store') }}" method="post">
                        @csrf
                        <div class="form-group">
                            <label> تاريخ الفاتورة</label>
                            <input name="order_date" id="order_date" type="date" value="@php echo date("Y-m-d"); @endphp"
                                class="form-control" value="{{ old('order_date') }}">
                            @error('notes')
                            ...
                            <select name="suuplier_code" id="suuplier_code" class="form-control select2">
                                ...
                                @if (@isset($suupliers) && !@empty($suupliers))
                                    @foreach ($suupliers as $info)
                                        <option @if (old('suuplier_code') == $info->suuplier_code) selected="selected" @endif
                                            value="{{ $info->suuplier_code }}"> {{ $info->name }} </option>
                                    @endforeach
                                @endif
                            </select>
                            @error('suuplier_code')
                            ...
                            <select name="store_id" id="store_id" class="form-control select2">
                                ...
                                @if (@isset($stores) && !@empty($stores))
                                    @foreach ($stores as $info)
                                        <option @if (old('store_id') == $info->id) selected="selected" @endif
                        ...
                        <div class="form-group text-center">
                            ...
                            <a href="{{ route('suppliers_orders_general_return.index') }}"
