        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <input type="hidden" id="ajax_search_load_add_permission_roles_sub_menu"
                        value="{{ route('permission_roles.load_add_permission_roles_sub_menu') }}">
                    <input type="hidden" id="ajax_search_load_add_permission_roles_sub_menues_actions"
                        value="{{ route('permission_roles.load_add_permission_roles_sub_menues_actions') }}">
                ...
                <div class="card-body">
                    @if (@isset($data) && !@empty($data))
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        لايوجد تحديث
                                    @endif
                                    <a href="{{ route('permission_roles.edit', $data['id']) }}"
                        ...
                        <!--  treasuries_delivery   -->
                        <div class="card-header">
                            <h3 class="card-title card_title_center">القوائم الرئيسية المضافة لصلاحية الدور (
                                {{ $data['name'] }} )
                                <button class="btn btn-sm btn-primary" data-toggle="modal"
                                    data-target="#Add_permission_main_menuesModal">اضافة قائمة </button>
                            </h3>
                        </div>
                        <div id="ajax_responce_serarchDiv">
                            @if (
                                @isset($permission_roles_main_menus) &&
                                    !@empty($permission_roles_main_menus) &&
                                    count($permission_roles_main_menus) > 0)
                                @php
                                    $i = 1;
                                @endphp
                                    ...
                                    <table id="example2" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('permission_roles.delete_permission_main_menues', $info->id) }}"
                                                        class="btn btn-sm btn-danger are_you_shue">حذف</a>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="3">
                                                    <p style="text-align: center; font-size: 1.4vw; color: brown"> القوائم
                                                        الفرعية المضافة لهذه القائمة الرئيسية

                                                        <button data-id="{{ $info->id }}"
                                                            class="btn btn-sm load_add_permission_roles_sub_menu btn-info">اضافة
                                                            قائمة فرعية </button>


                                                    </p>
                                                    @if (
                                                        @isset($info->permission_roles_sub_menu) &&
                                                            !@empty($info->permission_roles_sub_menu) &&
                                                            count($info->permission_roles_sub_menu) > 0)
                                                        <table id="example2" class="table table-bordered table-hover">
                                                            <thead class="bg-info">
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td>
                                                                            <button data-id="{{ $sub->id }}"
                                                                                class="btn btn-sm load_add_permission_roles_sub_menues_actions btn-success">اضافة
                                                                                صلاحيات مباشرة </button>

                                                                            <a href="{{ route('permission_roles.delete_permission_sub_menues', $sub->id) }}"
                                                                                class="btn btn-sm btn-danger are_you_shue">حذف</a>
                                                                        </td>
                                                                    </tr>
                                                                    <tr>
                                                                        <td colspan="3" style="text-align: center">
                                                                            @if (
                                                                                @isset($sub->permission_roles_sub_menues_actions) &&
                                                                                    !@empty($sub->permission_roles_sub_menues_actions) &&
                                                                                    count($sub->permission_roles_sub_menues_actions) > 0)
                                                                                @foreach ($sub->permission_roles_sub_menues_actions as $action)
                                                                                    <a style="    background-color: mediumvioletred;"
                                                                                        href="{{ route('permission_roles.delete_permission_sub_menues_actions', $action->id) }}"
                                                                                        class="btn btn-sm btn-primary are_you_shue">{{ $action->permission_sub_menues_actions_name }}
                                                                                        <i class="fa fa-trash"
                ...
                <div class="modal-body" style="background-color: white !important; color:black;">
                    <form action="{{ route('permission_roles.Add_permission_main_menues', $data['id']) }}" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <select required name="permission_main_menues_id[]" multiple id="permission_main_menues_id"
    ...
    <div class="modal fade  " id="load_add_permission_roles_sub_menuModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                ...
                <div class="modal-body" style="background-color: white !important; color:black;"
                    id="load_add_permission_roles_sub_menuModalBody">
    ...
    <div class="modal fade  " id="load_add_permission_roles_sub_menues_actionsModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-info">
                ...
                <div class="modal-body" style="background-color: white !important; color:black;"
                    id="load_add_permission_roles_sub_menues_actionsModalBody">
