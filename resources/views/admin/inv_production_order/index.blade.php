...
<a href="{{ route('inv_production_order.index') }}">  اوامر التشغيل</a>
@endsection
...
<div class="card">
   <div class="card-header">
      ...
      <input type="hidden" id="ajax_search_url" value="{{ route('inv_production_order.ajax_search') }}">
      <input type="hidden" id="ajax_show_more_detials_url" value="{{ route('inv_production_order.show_more_detials') }}">
      <a href="{{ route('inv_production_order.create') }}" class="btn btn-sm btn-success" >اضافة جديد</a>
   </div>
   <!-- /.card-header -->
   <div class="card-body">
      <div class="row">
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td>@if($info->is_approved==1)  معتمد
                           ...
                           @else
                           غير معتمد
                           <a href="{{ route('inv_production_order.do_approve',$info->id) }}" class="btn are_you_shue btn-sm  btn-success">اعتماد</a>
                           @endif
                        </td>
                        <td>@if($info->is_closed==1) مغلق ومرحل
                           ...
                           <a href="{{ route('inv_production_order.do_closes_archive',$info->id) }}" class="btn are_you_shue btn-sm  btn-success">اغلاق وترحيل</a>
                           @endif
                           @endif
                        </td>
                        <td >
                        ...
                        <td>
                           @if($info->is_closed==0)
                           <a href="{{ route('inv_production_order.edit',$info->id) }}" class="btn btn-sm  btn-primary">تعديل</a>
                           <a href="{{ route('inv_production_order.delete',$info->id) }}" class="btn are_you_shue btn-sm  btn-danger">حذف</a>
                           @endif
